...
class GamesPagingSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, com.msardevelopment.rawggameapp.data.model.apimodel.gamesbygenresresponse.Result> {
        ...
        return try {
            ...
            if(genres.isEmpty()) {
                response = rawgApi.getAllGames(Constants.API_KEY, position, params.loadSize)
            }
            else {
                response = rawgApi.getGamesByGenres(genres, Constants.API_KEY, position, params.loadSize)
            }
